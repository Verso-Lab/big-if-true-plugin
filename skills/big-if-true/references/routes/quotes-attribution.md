...
(the independence rule in SKILL.md sets the bar). When it stays
could_not_verify, name the recording or notes that would settle it.
...
| Company executives | earnings-call transcripts (the company's IR page; SEC 8-K exhibits via `evidence.py edgar`), press releases |
...
| Analyst notes, research reports | not public; the quote rests on the reporter — could_not_verify, say so plainly |
| Interviews conducted by the reporter | rest on the reporter — could_not_verify, say so plainly |
...
2. When the record is found, read around the quote: was it hedged,
   ...
   needs_review finding even when the words are exact.
...
4. Titles and affiliations are separate claims, checked against the
   ...
   `evidence.py wikidata "name"` for a first fix, LinkedIn as a last
