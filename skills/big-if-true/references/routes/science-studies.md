...
- `python3 scripts/evidence.py crossref "title words or topic"` returns
  DOIs, journals, dates and authors. `evidence.py doi 10.xxxx/…` resolves
  a DOI the text or a press release gives.
- Medicine and life sciences: `evidence.py pubmed "query"` (PMIDs, open at
  pubmed.ncbi.nlm.nih.gov/PMID). Preprints: `evidence.py arxiv "query"`;
...
- Confirm the person exists and holds the stated position (university
  page, ORCID at orcid.org, Google Scholar, `evidence.py wikidata`). "Scientists say" with no
